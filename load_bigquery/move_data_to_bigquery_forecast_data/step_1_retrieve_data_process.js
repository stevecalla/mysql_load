...
const { forecastDataQuery, forecastSummaryMetricsQuery } = require('./query_data');
...
async function deleteArchivedFiles() {
    ...
    const directoryName  = `bigquery_forecast_archive`;
...
async function moveFilesToArchive() {
    ...
    try {
        // List all files in the directory
        let directoryName = `bigquery_forecast`;
        const sourcePath = `${os_path}${directoryName}`;
        // await create_directory('bigquery_forecast');
        
        const files = fs.readdirSync(sourcePath);
        console.log(files);

        // Create the "archive" directory if it doesn't exist
        directoryName  = `bigquery_forecast_archive`;
...
async function export_results_to_csv_fast_csv(results, file_name, i) {
    ...
    const directoryName  = `bigquery_forecast`;
    const directoryPath = await create_directory(directoryName);
...
async function execute_retrieve_data() {
    ...
    try {
        ...
        console.log(`\nSTEP 3: PULL SQL DATA FROM FORECAST DATA TABLE`);
        ...
        const getData = [
            ...
            {
                poolName: localForecastDbConfig,
                fileName: 'forecast_metrics_data',
                query: forecastSummaryMetricsQuery
            },
        ];
